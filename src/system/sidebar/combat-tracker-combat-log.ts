...
export default class CombatLog {
  ...
  declare movementIndex: number
  declare movementOptions: IMovementOption[]
  declare strainLog: IStrainLogEntry[]
  ...
  getContext(combatant: foundryAny) {
  ...
  updateDistance(combatant: foundryAny) {
    const movementHistory = combatant.token.movementHistory;

    this.distance = movementHistory.reduce(
  ...
  changeMovementIndex(newIndex: number) { this.movementIndex = newIndex; }
  ...
  static getMovementOptions(distance: number): IMovementOption[] {
  ...
  getMovementStrainLog(): IStrainLogEntry[] {
    if (!this.movementOptions.length) return [];

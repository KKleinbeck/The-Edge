...
class ProficiencyData extends DataModelComponent {
  ...
  getProficiencyDiceThresholds(proficiency: string): IDiceThreshold[] {
    const proficiencyData = Object.values(this.proficiencies).find(
      (profClass) => proficiency in profClass,
    )[proficiency];
  ...
  get proficiencyDiceParameter(): IDiceParameters {
    // Placeholder
    return {
      critDice: [1],
      critBonus: 5,
  ...
  async rollProficiencyCheck(
    promptResult: IProficiencyPromptResult,
    onSubmitCallback?: Function,
  ): Promise<IRollResult> {
    const proficiencyData = this.getProficiencyDiceThresholds(
      promptResult.proficiency,
    );
    ...
    const rollResult: IRollResult =
      await DiceServer.proficiencyCheck(diceServerConfig);

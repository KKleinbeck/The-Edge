...
export default class GrenadePicker extends HandlebarsApplicationMixin(
  ApplicationV2,
) {
  ...
  constructor(options) {
    ...
    this.targetPosition = {
      x: options.mousePosition.x,
      y: options.mousePosition.y,
    ...
    for (const token of tokens) {
      ...
      const grenades = token.actor.itemTypes["Consumables"].filter((x) => {
        return "grenade" == x.system.current_type;
      });
      if (grenades.length >= 1) {
        ...
        const size = (Object.values(THE_EDGE.sizes) as number[]).find(
          (value: number, _index: number, _obj: unknown[]) =>
            value > token.actor.system.height,
        );
  ...
  hasContent() {
    return this.actorOptions.length >= 1;
  }
  ...
  _getDistance(mousePosition, token) {
    ...
    return (
      factor *
      Math.hypot(
        mousePosition.x - token.x - offset,
        mousePosition.y - token.y - offset,
      )
    );
  }
  ...
  static async _rollCheck(_event, target: HTMLElement) {
    ...
    const rollQuery: IProficiencyRollQuery = {
      ...
      transmit: false,
      modifier: actorOptions.modifier,
    };
    DialogProficiency.start(rollQuery, (rollDetails: IProficiencyRollMessage) =>
      this._onRollCompleted(rollDetails, target),
    );
    this.close();
    return;
  }
  ...
  _onRollCompleted(rollDetails: IProficiencyRollMessage, target: HTMLElement) {
    ...
    const rollOutcome = ProficiencyConfig.rollOutcome(
      "throwing",
      rollDetails.quality,
    );
    foundry.utils.mergeObject(rollDetails, {
      rollOutcome,
      titleDetails: chosenGrenade.name,
    });
    const chatServerConfig: IChatServerConfig = {
      speaker: { scene: canvas.scene.id, token: token.id },
    };
    ...
    const payload: _IGrenadePickerPayload = {
      actor: token.actor,
      rollDetails: rollDetails as _IExtendedRollMessage,
  ...
  _getTokenAndGrenade(target: HTMLElement) {
    ...
    const grenadeIndex = +(
      target.parentNode.querySelector(".grenade-selection") as HTMLInputElement
    ).value;
  ...
  static async createGrenadeTile(payload: _IGrenadePickerPayload) {
    ...
    const details = {
      nameGrenade: rollDetails.titleDetails,
      nameActor: payload.actor.name,
      grenade,
      grenadeTileId: grenadeTile.id,
  ...
  static _createGrenadePosition(
    ...
    if (quality >= 0) {
      const angle =
        (Math.PI * (45 * rollOutcome.dir + 40 * Math.random() - 20)) / 180;
      position.x = Math.floor(targetPosition.x + dist * Math.sin(angle));
      position.y = Math.floor(targetPosition.y - dist * Math.cos(angle));
    } else {
      const angle =
        Math.atan2(
          tokenPosition.y - targetPosition.y,
          tokenPosition.x - targetPosition.x,
        ) +
        (Math.PI * (180 * rollOutcome.dir + 40 * Math.random() - 20 + 90)) /
          180;

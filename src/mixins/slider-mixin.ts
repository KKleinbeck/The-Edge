...
export default function SliderMixin<T extends intype>(
  BaseApplication: T,
): outtype<T> {
  class SliderMixinClass extends BaseApplication {
    ...
    static getSliderValuesFromElement(
      element: Element,
    ...
    _attachEffectListeners(): void {
      this.element
        ...
        ?.forEach((x: Element) =>
          x.addEventListener("click", (ev: Event) =>
            this._sliderClickedEvent(ev),
          ),
        );
    }

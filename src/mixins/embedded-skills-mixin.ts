...
export default function EmbeddedSkillMixin<T extends intype>(
  BaseApplication: T,
): outtype<T> {
  return class EmbeddedSkill extends BaseApplication {
    ...
    async updateSkills(
      ...
      await this.document.update(
        { "system.embeddedSkills": counters },
        { render: false },
      );
    }
    ...
    attachSkillEffectListeners(element: Element | undefined = undefined): void {
      ...
      const skillNameElements = element.querySelectorAll(
        ".embedded-skills-name-hook",
      );
      for (const skillElement of skillNameElements) {
        skillElement.addEventListener("change", (ev) =>
          this._onChangeSkillName(ev),
        );
      }
    }
    ...
    static async _onSkillControl(
      ...
      switch (target.dataset.subaction) {
        ...
        case "edit":
          const newEffect = await DialogDynamicModifier.prompt(
            skills[index].effect,
          );

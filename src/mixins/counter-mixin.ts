...
export default function CounterMixin<T extends intype>(
  ...
  return class CounterHandler extends BaseApplication {
    ...
    attachCounterEffectListeners(
      ...
      const counterNameElements =
        element.querySelectorAll(".counter-name-hook");
      for (const counter of counterNameElements) {
        counter.addEventListener("change", (ev) =>
          this._onCounterChange(ev, "name"),
        );
      }
    ...
    static async _onCounterControl(
      this: CounterHandler,
      _event: Event,
      target: HTMLElement,
    ): Promise<void> {
      ...
      switch (target.dataset.subaction) {
        ...
        case "use":
          ...
          counters[index].value =
            counters[index].value < level ? level : level - 1;
          break;
      }

...
export default class DialogProficiency extends CheckDialog {
  ...
  static async start(
    checkData: IProficiencyRollQuery,
    onSubmitCallback?: Function,
  ): Promise<DialogV2> {
    const proficiencyData = checkData.actor.system.getProficiencyDiceThresholds(
      checkData.proficiency,
    );
    var threshold = 0;
    ...
    const html = await renderTemplate(template, {
      chance: Aux.asChance(
        Aux.proficiencySuccessChance(
          threshold,
          checkData.actor.system.proficiencyDiceParameter,
        ),
        true,
      ),
      modifierValue: checkData.modifier ?? 0,
      maxStrain:
        proficiencyData.filter((x) => x.name == "proficiency")[0].threshold +
        strainReduction,
    ...
    return new DialogProficiency(checkData, {
      window: {
        title:
          LocalisationServer.localise(checkData.proficiency, "proficiency") +
          " " +
      ...
      submit: async (result: any, dialog: DialogProficiency): Promise<void> => {
        if (result === "cheat")
          ...
          DialogProficiency.rollCallback(
            dialog,
            checkData,
            result,
            onSubmitCallback,
  ...
  static async rollCallback(
    ...
    const proficiencyPromptResult: IProficiencyPromptResult =
      foundry.utils.mergeObject(checkData, promptResult);
    await checkData.actor.system.rollProficiencyCheck(
      proficiencyPromptResult,
      onSubmitCallback,
    );
  ...
  onValueChanged(_id: string, _value: number): void {
  ...
  _onChanceChanged() {
    ...
    for (const data of this.checkData.actor.system.getProficiencyDiceThresholds(
      this.checkData.proficiency,
    )) {
    ...
    var chance = Aux.proficiencySuccessChance(
      threshold,
      this.checkData.actor.system.proficiencyDiceParameter,
    );

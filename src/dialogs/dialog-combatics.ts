...
export default class DialogCombatics extends CheckDialog {
  ...
  static async start(checkData: IAttackRollQuery, weaponId: string) {
    ...
    const handToHandLevel: number =
      checkData.actor.system.weapons.general["Hand-to-Hand combat"].value;
    const strainMaxUseReduction =
      checkData.actor.system.strain.maxUseReduction.status;
    const html = await renderTemplate(template, {
      chance: Aux.asChance(
        Aux.attackSuccessChance(
          checkData.threshold,
      ...
      maxStrain: THE_EDGE.combatConfig.handToHandMaxStrain(
        handToHandLevel,
  ...
  async cheatCallback(): Promise<void> {
    ...
    const threshold: number =
      this.checkData.threshold + (Object.values(sliderValues).sum() as number);

    const dieResult: number = await Aux.promptInput(
      LocalisationServer.localise("Cheat attack roll", "dialog"),
    );
    const diceParameters: IAttackDiceParameters =
  ...
  async attackCallback(): Promise<void> {
    ...
    const threshold: number =
      this.checkData.threshold + promptResult.modifier + promptResult.strain;
    ...
    const attackRollResult: IAttackRollResult =
      await this.checkData.actor.system.rollAttackCheck(prompt);
  ...
  _onChanceChanged() {
    ...
    var chance = Aux.attackSuccessChance(
      threshold,
      THE_EDGE.combatConfig.attackDiceParameters(this.checkData.actor),
    );

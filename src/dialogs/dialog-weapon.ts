...
export default class DialogWeapon extends DialogV2 {
  ...
  static async start(checkData: IDialogWeaponData): Promise<DialogV2 | null> {
    ...
    const config = {
      ...
      window: {
        title:
          checkData.weapon.name + " " + LocalisationServer.localise("Check"),
      },
  ...
  static _extendCheckData(
    checkData: IDialogWeaponData,
  ): IDialogWeaponDataExtended {
    const extension = {
      distance: DialogWeapon._getDistance(
        checkData.token,
        checkData.targetIds,
        checkData.sceneId,
      ),
      smallestSize: DialogWeapon._getSmallestSize(
        checkData.targetIds,
        checkData.sceneId,
      ),
  ...
  static async _setupContent(
    checkData: IDialogWeaponDataExtended,
  ): Promise<string> {
  ...
  static _rollAttackButton(): Record<string, any> {
    return {
      ...
      callback: async (_event, _button, dialog: DialogWeapon) => {
        const parseResult = DialogWeapon._onRollParseSheet(
          dialog.element,
          dialog.checkData,
        );
        parseResult.fireModeDetails =
          dialog.checkData.weapon.system.fireModes[parseResult.fireModeIndex];

        parseResult.nShots =
          await DialogWeapon._onRollUpdateAmmunitionGetNumberShots(
            dialog.checkData,
            parseResult,
          );
        const rollDamageOutcome: _IRollDamageOutcome =
          await DialogWeapon._onRollRollDamage(dialog.checkData, parseResult);

        DialogWeapon._onRollApplyDamageAndPost(
          dialog.checkData,
          parseResult,
          rollDamageOutcome,
        );
  ...
  static _getDistance(
    ...
    const distances = targets.map(
      (target) => factor * Aux.tokenDistance(aggressor, target),
    );
    return Math.max(...distances);
  }

  static _getSmallestSize(
    targetIds: string[],
    sceneId: string,
  ): TSize | undefined {
    // get the scene
    ...
    // @ts-expect-error
    return Object.entries(THE_EDGE.sizes).find(
      ([_, value]) => (value as number) > smallest,
    )[0];
  }
  ...
  static _onRollParseSheet(
    ...
    const labelFor: IAttackWeaponLabels = {
      ...
      fireMode:
        DialogWeapon._safeGetByName(element, "FireSelector") ??
        checkData.weapon.system.fireModes[0].name,
      movement: DialogWeapon._safeGetByName(
        element,
        "MovementSelector",
      ) as TMovement,
      precision: (DialogWeapon._safeGetByName(element, "PrecisionSelector") ??
        "aimed") as TPrecision,
      range: DialogWeapon._onRollGetRange(element, checkData.distance),
      size: (DialogWeapon._safeGetByName(element, "SizeSelector") ??
        checkData.smallestSize) as TSize,
    };

    const [modifiers, modifierIndicies] =
      DialogWeapon._onRollGetModifiersAndIndices(element, checkData, labelFor);
    ...
    return {
      ...
      modifiers,
      ...modifierIndicies,
      fireModeDetails: undefined,
      nShots: 0, // Placeholders
      vantage: DialogWeapon._safeGetByName(
        element,
        "VantageSelector",
      ) as TVantage,
    };
  ...
  static _onRollGetRange(
    ...
    return (
      (DialogWeapon._safeGetByName(element, "RangeSelector") as TDistance) ??
      "less_2m"
    );
  }
  ...
  static _onRollGetModifiersAndIndices(
    ...
    const fireModeIndex = checkData.weapon.system.fireModes.findIndex(
      (x) => x.name === labels.fireMode,
    );
    ...
    return [
      {
        ...
        fireModeModifier:
          checkData.weapon.system.fireModes[fireModeIndex].precisionPenalty[
            pIndex
          ],
        ...
        weaponProfLevel: checkData.actor.system.getWeaponPlOfWeapon(
          checkData.weapon.id,
        ),
      },
  ...
  static async _onRollUpdateAmmunitionGetNumberShots(
    checkData: IDialogWeaponDataExtended,
    parseResult: _IParseResult,
  ): Promise<number> {
    const ammunition = checkData.actor.items.get(
      checkData.weapon.system.ammunitionID,
    );
    ...
    await ammunition.update({
      "system.capacity.value": ammuCapa.value - ammuCost,
    });

    return Math.floor(
      ((ammuCost + 0.1) * fireModeModifier.dices) / fireModeModifier.cost,
    );
  }
  ...
  static async _onRollRollDamage(
    ...
    const damageRoll: string =
      parseResult.fireModeDetails.damage +
      ` + ${ammunition.system.damage.bonus}`;
    ...
    Hooks.call("onModifierEvent", "rollAttackCheck-Prior", {
      actor: checkData.actor,
      prompt,
      weaponId: checkData.weapon.id,
    });
    const attackRollResult: IAttackRollResult =
      await checkData.actor.system.rollAttackCheck(prompt);
    DialogWeapon._dispatchActionHook(checkData, parseResult);
    Hooks.call("onModifierEvent", "rollAttackCheck-Posterior", {
      actor: checkData.actor,
      attackRollResult,
      prompt,
      weaponId: checkData.weapon.id,
    ...
    return {
      penetration: ammunition.system.penetration,
      prompt,
      attackRollResult,
    };
  }
  ...
  static async _onRollApplyDamageAndPost(
    checkData: IDialogWeaponDataExtended,
    parseResult: _IParseResult,
    rollDamageOutcome: _IRollDamageOutcome,
  ) {
    ...
    if (rollDamageOutcome.attackRollResult.failEvent) {
      ChatServer.transmitEvent(
        "CRIT FAIL EVENT",
        {
          event: rollDamageOutcome.attackRollResult.failEvent,
          check: "Combat check",
        },
        chatServerConfig,
      );
  ...
  static _dispatchActionHook(
    checkData: IDialogWeaponDataExtended,
    parseResult: _IParseResult,
  ) {
    const payload: ITheEdgeActionPayload = {
      actionType:
        parseResult.labels.precision == "aimed"
          ? "weapon check aimed"
          : "weapon check",
      actionCost: THE_EDGE.combatConfig.weaponAttackActionCost(
        parseResult.labels.precision,
      ),
      actor: checkData.actor,
    };
  ...
  static _extractChatServerConfig(
    checkData: IDialogWeaponDataExtended,
  ): IChatServerConfig {
    return {
      speaker: {
  ...
  static _safeGetByName(
    element: HTMLElement,
    name: string,
  ): string | undefined {
    const target: HTMLSelectElement | HTMLInputElement | null =

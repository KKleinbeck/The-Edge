...
export default class DialogEditWounds extends DialogV2{
  ...
  static async prompt(
    ...
    return await super.prompt(
      foundry.utils.mergeObject(this.DEFAULT_CONFIG, config)
    );
  }
  ...
  static _setupCallback() {
    return {
      ...
      callback: (_event, _button, dialog): Partial<IWoundDetails> => {
        const result = {};
        for (const entry of ["source", "status", "damage", "bleeding"]) {
          const element: HTMLInputElement = dialog.element.querySelector(`.${entry}-hook`);

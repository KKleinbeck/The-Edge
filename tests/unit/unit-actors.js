...
export default function registerUnitTestsForActors(apiHandler) {
  async function actorAlterMaxHealth() {
    ...
    for (let i = 0; i < samples.length; i++)
      assert(actualMaxHealth[i] == expectedMaxHealth[i]);
  }
  ...
  async function actorApplyStrain() {
    ...
    const expectedResults = [
      { strainChange: 0, strainLevel: 0 },
      { strainChange: 25, strainLevel: 1 },
      { strainChange: 50, strainLevel: 2 },
      { strainChange: 100, strainLevel: 4 },
      { strainChange: 100, strainLevel: 4 },
    ];
    const actualResults = [];
  ...
  TestRegistry.registerTest(
    actorApplyDamageDying,
    "Actor Apply Damage when Dying",
    "unit",
  );
  ...
  async function actorApplyStrainRepeated() {
    ...
    const expectedResults = [
      { strainChange: 15, strainLevel: 0, value: 15 },
      { strainChange: 15, strainLevel: 1, value: 30 },
      { strainChange: 15, strainLevel: 2, value: 45 },
      { strainChange: 15, strainLevel: 3, value: 60 },
  ...
  TestRegistry.registerTest(
    actorApplyStrainRepeated,
    "Actor Apply Strain Repeated",
    "unit",
  );
  ...
  async function actorCombatics() {
    ...
    const samples = [
      { str: 0, crd: 0, h2h: 0, gen: 0 },
      { str: 0, crd: 0, h2h: 0, gen: 10 },
      { str: 2, crd: 2, h2h: 0, gen: 10 },
      { str: 1, crd: 3, h2h: 0, gen: 10 },
      { str: 3, crd: 4, h2h: 0, gen: 10 },
      { str: 8, crd: 8, h2h: 8, gen: 10 },
    ];
    const expectedResults = [
      { general: 0, combatics: 0, damage: "1d0+0" },
      { general: 0, combatics: 5, damage: "1d0+0" },
      { general: 2, combatics: 6, damage: "1d4+2" },
      { general: 2, combatics: 6, damage: "1d4+1" },
      { general: 3, combatics: 6, damage: "1d7+3" },
      { general: 8, combatics: 13, damage: "1d16+8" },
    ];
    const actualResults = [];
  ...
  async function actorDetermineBleeding() {
    const samples = [
      { damage: 0, threshold: 10 },
      { damage: 10, threshold: 10 },
      { damage: 25, threshold: 10 },
    ];
    ...
    for (let i = 0; i < samples.length; i++)
      actualResults[i] >= expectedResults[i];
  }
  TestRegistry.registerTest(
    actorDetermineBleeding,
    "Actor Apply Damage",
    "unit",
  );
  ...
  async function actorLevelingAttributes() {
    ...
    for (let i = 0; i < samples.length; i++)
      assert(actualCosts[i] == expectedCosts[i]);
  }
  TestRegistry.registerTest(
    actorLevelingAttributes,
    "Actor Leveling Attributes",
    "unit",
  );

  async function actorLevelingProficiencies() {
    const samples = [2, 4, 6, 8];
    const expectedResults = samples.map((x) =>
      accumulate(THE_EDGE.profCost, x),
    );
    const actualResults = [];
  ...
  TestRegistry.registerTest(
    actorLevelingProficiencies,
    "Actor Leveling Proficiencies",
    "unit",
  );
  ...
  async function actorSpeeds() {
    ...
    const expectedSpeeds = [
      { stride: 0, run: 0, sprint: 0 },
      { stride: 5, run: 7, sprint: 8 },
      { stride: 5, run: 7, sprint: 9 },
      { stride: 6, run: 9, sprint: 12 },
      { stride: 7, run: 11, sprint: 16 },
    ];
    const actualSpeeds = [];
  ...
  async function actorWeaponProficiencyLevel() {
    const samples = [
      { general: 0, SABs: 0, LMGs: 0 },
      { general: 10, SABs: 0, LMGs: 0 },
      { general: 0, SABs: 10, LMGs: 0 },
      { general: 10, SABs: 10, LMGs: 0 },
      { general: 10, SABs: 9, LMGs: 0 },
      { general: 11, SABs: 9, LMGs: 0 },
      { general: 10, SABs: 10, LMGs: 4 },
    ];
    const expectedResults = [
      { SABvalue: 0, SABactual: 0 },
      { SABvalue: 0, SABactual: 5 },
      { SABvalue: 0, SABactual: 0 },
      { SABvalue: 10, SABactual: 10 },
      { SABvalue: 9, SABactual: 9 },
      { SABvalue: 9, SABactual: 10 },
  ...
  TestRegistry.registerTest(
    actorWeaponProficiencyLevel,
    "Actor Weapon Proficiency Level",
    "unit",
  );

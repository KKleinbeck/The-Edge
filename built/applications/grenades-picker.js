...
export default class GrenadePicker extends HandlebarsApplicationMixin(
  ...
  constructor(options) {
    ...
    for (const token of tokens) {
      ...
      if (grenades.length >= 1) {
        ...
        const size = Object.values(THE_EDGE.sizes).find(
          (value, _index, _obj) => value > token.actor.system.height,
        );
  ...
  static async _rollCheck(_event, target) {
    ...
    DialogProficiency.start(rollQuery, (rollDetails) =>
  ...
  _getTokenAndGrenade(target) {
    ...
    const grenadeIndex =
      +target.parentNode.querySelector(".grenade-selection").value;

...
export default class GrenadePicker extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    _getDistance(mousePosition, token) {
        ...
        return (factor *
            Math.hypot(mousePosition.x - token.x - offset, mousePosition.y - token.y - offset));
    }
    _getDistanceModifier(distance, size) {
        if (distance <= THE_EDGE.sizeFreeThrow[size])
            return 30;
        if (distance <= 10)
            return Math.floor(10 - distance);
        if (distance <= 20)
            return Math.floor((10 - distance) / 2);

...
function processLanguage(matches, chatData) {
    ...
    const usersWitoutTranslation = _translateForProficientUsers(language, matches[2], newMessage);
...
function _getSpeakingActors(language) {
    const speakingActors = game.actors.map(actor => {
        const learnedLevels = actor.itemTypes["Languageskill"].filter(x => x.name.toLowerCase() == language).map(x => x.system.level);
        const nativeLevel = (language == actor.system.nativeLanguage.toLowerCase() ?
            6 : 0);
        return {
...
function _mapLevelsToUsers(userProficiency) {
    ...
    for (const [userID, level] of Object.entries(userProficiency)) {
        if (level in usersByLevel)
            usersByLevel[level].push(userID);
        else
            usersByLevel[level] = [userID];
    }
    return usersByLevel;
}
function _createMessageForLevel(level, language, message, newMessage) {
    const translatedMessage = _getLevelTranslation(level, language, message, newMessage);

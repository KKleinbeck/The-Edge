...
async function heroTokenAttributeCheck(chatMsgCls, actor, sys) {
    if (sys.outcome == "Success")
        updateAttributeCheck(chatMsgCls, actor, sys, 1);
    else
        updateAttributeCheck(chatMsgCls, actor, sys, sys.threshold);
}
async function updateAttributeCheck(chatMsgCls, actor, sys, newResult) {
    ...
    const newContent = await renderTemplate("systems/the_edge/templates/chat/attribute_check.html", newSys);
...
function heroTokenProficiencyCheck(chatMsgCls, actor, sys) {
    ...
    if (sys.netOutcome < 0) {
        for (let i = 0; i < 3; ++i) {
            sys.diceResults[i] = Math.min(sys.thresholds[i] + Math.floor(modificator / 3), 19);
        }
    }
    else {
...
async function updateProficiencyCheck(chatMsgCls, actor, sys, newResults) {
    ...
    const newContent = await renderTemplate("systems/the_edge/templates/chat/proficiency_check.html", sys);
    updateChatMessage(chatMsgCls, newContent, newSys);
}
function heroTokenWeaponCheck(chatMsgCls, actor, sys, index) {
    ...
    if (newResults[index].hit)
        ...
        newResults[index].res = sys.threshold;
    updateWeaponCheck(chatMsgCls, actor, sys, newResults, index);
}
async function updateWeaponCheck(chatMsgCls, actor, sys, newResults, index) {
    ...
    const newContent = await renderTemplate("systems/the_edge/templates/chat/weapon_check.html", sys);

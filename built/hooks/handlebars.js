...
export default function () {
    Handlebars.registerHelper({
        ...
        checkNotEmpty: (a) => { if (!a)
            return false; return a.length !== 0; },
        ...
        getEntries: (a, ...args) => {
            ...
            return argsWithoutCall.reduce((obj, key) => (obj && obj[key] !== 'undefined') ? obj[key] : undefined, a);
        },
        log: (a) => console.log(a),
        add: (a, b) => { return +a + +b; },
        sub: (a, b) => { return a - b; },
        div: (a, b) => { if (b == 0)
            return undefined; return a / b; },
        mul: (a, b) => { return a * b; },
        and: (a, b) => { return a && b; },
        or: (a, b) => { return a || b; },
        ...
        checkHasMultipleStacked: (systemData) => {
            if ("quantity" in systemData && systemData.quantity > 1)
                return true;
            return false;
        },
        getRangeModifier: (rangeChart, distance) => {
            if (distance < 2)
                ...
                return `(${rangeChart["less_20m"][0]} / ${rangeChart["less_20m"][1]})`;
            else if (distance < 200)
                return `(${rangeChart["less_200m"][0]} / ${rangeChart["less_200m"][1]})`;
            else if (distance < 1000)
                return `(${rangeChart["less_1km"][0]} / ${rangeChart["less_1km"][1]})`;

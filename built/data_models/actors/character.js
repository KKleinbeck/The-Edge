...
const CharacterDataParent = generateDataModelWithComponents(ActorEffectData, AttributeData, CharacterBaseData, CombatantData, CreditData, HumanoidData, ProficiencyData, StatusEffectData, WeaponData);
export default class CharacterData extends CharacterDataParent {
    ...
    _initialize(options = {}) {
        ...
        this.strain.max.value = this.strain.max.baseline + this.strain.max.status +
            this.attributes.end.value - Math.floor((this.age - 21) / 3) - this.bloodLoss.value;
        ...
        if (coreName.split(".")[1] === "weapons") {
            if (coreName.includes("Hand-to-Hand combat")) {
                if (newVal > this.combaticsGeneralPl) {
                    const msg = LocalisationServer.parsedLocalisation("Core Value combatics too small", "Notifications", { level: newVal, basic: this.combaticsGeneralPl });
                    ui.notifications.notify(msg);
                    return;
                }
            }

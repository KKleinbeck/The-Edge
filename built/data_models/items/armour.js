...
const { ArrayField, NumberField, ObjectField, SchemaField, StringField } =
  foundry.data.fields;
export default class ArmourData extends generateDataModelWithComponents(
  ...
  async protect(damage, penetration, damageType, location, protectionLog) {
    ...
    if (damage <= protection.threshold) {
      update["system.structurePoints"] = Math.max(
        0,
        this.structurePoints - damage,
      );
      ...
      update["system.structurePoints"] = Math.max(
        0,
        this.structurePoints - protection.threshold,
      );

...
export default class DialogWeapon extends DialogV2 {
  ...
  static _rollAttackButton() {
    return {
      ...
      callback: async (_event, _button, dialog) => {
        ...
        const rollDamageOutcome = await DialogWeapon._onRollRollDamage(
          dialog.checkData,
          parseResult,
        );
  ...
  static _onRollParseSheet(element, checkData) {
    const labelFor = {
      ...
      precision:
        DialogWeapon._safeGetByName(element, "PrecisionSelector") ?? "aimed",
      range: DialogWeapon._onRollGetRange(element, checkData.distance),
      size:
        DialogWeapon._safeGetByName(element, "SizeSelector") ??
        checkData.smallestSize,
    };
  ...
  static async _onRollApplyDamageAndPost(
    ...
    const details = DialogWeapon._generateWeaponCheckData(
      checkData,
      parseResult,
      rollDamageOutcome,
    );

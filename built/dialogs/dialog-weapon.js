...
export default class DialogWeapon extends DialogV2 {
    ...
    static async start(checkData) {
        ...
        const config = {
            ...
            window: {
                title: checkData.weapon.name + " " + LocalisationServer.localise("Check"),
            },
    ...
    static _rollAttackButton() {
        return {
            ...
            callback: async (_event, _button, dialog) => {
                ...
                parseResult.nShots =
                    await DialogWeapon._onRollUpdateAmmunitionGetNumberShots(dialog.checkData, parseResult);
    ...
    static _onRollGetRange(element, distance) {
        ...
        return (DialogWeapon._safeGetByName(element, "RangeSelector") ??
            "less_2m");
    }
    ...
    static async _onRollRollDamage(checkData, parseResult) {
        const ammunition = checkData.actor.items.get(checkData.weapon.system.ammunitionID);
        const damageRoll = parseResult.fireModeDetails.damage +
    ...
    static async _onRollApplyDamageAndPost(checkData, parseResult, rollDamageOutcome) {
        ...
        if (rollDamageOutcome.attackRollResult.failEvent) {
            ChatServer.transmitEvent("CRIT FAIL EVENT", {
    ...
    static _dispatchActionHook(checkData, parseResult) {
        const payload = {
            actionType: parseResult.labels.precision == "aimed"
                ? "weapon check aimed"
                : "weapon check",

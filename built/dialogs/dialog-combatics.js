...
export default class DialogCombatics extends CheckDialog {
  ...
  async cheatCallback() {
    ...
    const threshold =
      this.checkData.threshold + Object.values(sliderValues).sum();
    const dieResult = await Aux.promptInput(
      LocalisationServer.localise("Cheat attack roll", "dialog"),
    );
    const diceParameters = THE_EDGE.combatConfig.attackDiceParameters(

...
THE_EDGE.attrCost = (n) => {
    return 10 * Math.floor(12 + 8 * Math.pow(1.2, n));
};
THE_EDGE.profCost = (n) => {
    return 5 * Math.floor(10 + 4 * Math.pow(1.2, n));
};
THE_EDGE.sizes = {
    tiny: 30,
    small: 130,
...
THE_EDGE.movementModifiers = {
    stationary: [0, 0],
    moderate: [-1, -1],
    fast: [-2, -2],
    erradic: [-4, -4],
};
THE_EDGE.coverModifiers = {
    "no cover": 0,
    "half cover": -2,
    "three quarters": -4,
    "full cover": -20,
};
THE_EDGE.consumables_subtypes = [
    "Food",
    "Grenade",
    "SkinPack",
    "FleshPack",
...
THE_EDGE.combat_damage_types = ["energy", "kinetic", "elemental"];
    ...
    switch (damageType) {
        case "energy":
            return {
                abrasion: 10,
                "light burn": damage,
                "strong burn": Math.max(0, Math.ceil((damage * (damage - 10)) / 10)),
            };
        case "kinetic":
            return {
                abrasion: 10,
                laceration: damage,
                fracture: Math.max(0, Math.ceil((damage * (damage - 10)) / 10)),
            };
        case "elemental":
            return {
                "light burn": damage,
                "strong burn": Math.max(0, (damage * (damage - 10)) / 10),
            };
        case "fall":
        case "impact":
            return {
                abrasion: 20,
                laceration: Math.ceil(damage / 2),
                fracture: Math.max(0, Math.ceil((damage * (damage - 10)) / 10)),
            };
        case "HandToHand":
            return {
...
THE_EDGE.fallDamageRoll = (height) => {
    return `${height}d12 + ${4 * height - 22}`;
};
THE_EDGE.fallDamageWoundCount = (height) => {
    return Math.floor(height / 2);
};
THE_EDGE.impactDamageRoll = (speed) => {
    return `${speed}d${speed}+${speed - 30}`;
};
THE_EDGE.impactDamageWoundCount = (speed) => {

...
export default function EffectModifierMixin(BaseApplication) {
    return class EffectModifier extends BaseApplication {
        ...
        _modifyEffect(event) {
            ...
            if (modifiers[index].group !== "dynamicModifiers" &&
        ...
        _getModifierData(target) {
            ...
            if (entry == "field" &&
                THE_EDGE.isDynamicModifier(result[entry])) {
                result.value = THE_EDGE.dynamicModifierDefaults(result.field);
            }
            return result;
        }
        ...
        attachEffectListeners() {
            this.element
                .querySelectorAll(".modifier-hook")
                ?.forEach((x) => x.addEventListener("change", (ev) => this._modifyEffect(ev)));
        }
    };
}
